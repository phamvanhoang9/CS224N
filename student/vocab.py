...
from docopt import docopt # Command-line interface description language
from itertools import chain # chain(*iterables) --> chain object
import json
import torch
...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        if word2id:
            ...
            self.word2id = dict() # self.word2id is a dictionary mapping words to indices, e.g. {'<pad>': 0, '<s>': 1, '</s>': 2, '<unk>': 3}
            self.word2id['<pad>'] = 0   # Pad Token
            self.word2id['<s>'] = 1 # Start Token
            self.word2id['</s>'] = 2    # End Token
            self.word2id['<unk>'] = 3   # Unknown Token
        self.unk_id = self.word2id['<unk>']
        self.id2word = {v: k for k, v in self.word2id.items()} # self.id2word is a dictionary mapping indices to words, e.g. {0: '<pad>', 1: '<s>', 2: '</s>', 3: '<unk>'}
